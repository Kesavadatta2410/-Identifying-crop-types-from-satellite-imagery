...
from PIL import Image  # For reading RGB image files
...
# ==================== DATASET ====================
class EuroSATSpectralDataset(Dataset):
    def __init__(self, data_dir, transform=None):
        self.data_dir = Path(data_dir)
        self.transform = transform
        
        # Try to find the RGB data directory
        # Structure: eurosat_data/2750/ClassName/*.jpg
        possible_paths = [
            self.data_dir / "2750",
            self.data_dir / "EuroSAT" / "2750",
            self.data_dir / "EuroSATallBands" / "2750",
        ]
        ...
        if self.actual_data_dir is None:
             raise FileNotFoundError(f"Could not find EuroSAT RGB data in {data_dir}")
    ...
    def __getitem__(self, idx):
        ...
        # Read RGB image and resize to target size
        try:
            image = Image.open(img_path).convert('RGB')
            # Resize to CONFIG image_size (224)
            image = image.resize((CONFIG['image_size'], CONFIG['image_size']), Image.BILINEAR)
        except Exception as e:
            print(f"Error reading {img_path}: {e}")
            # Return dummy image to avoid crash
        ...
        # Don't convert to tensor here - let transforms handle it
        # This allows augmentation transforms to work
        if self.transform:
...
# ==================== MODEL ====================
def get_spectral_model(num_classes, in_channels=3, dropout_rate=0.3):
    print(f"Loading EfficientNet-B0 for {in_channels} channels (RGB)...")
    model = models.efficientnet_b0(pretrained=True)
    
    # Since we're using RGB (3 channels), we don't need to modify the first layer
    # Just modify the classifier for our number of classes with dropout
...
def plot_metrics(history):
    ...
    # Kappa Score (if available)
    if 'val_kappa' in history and history['val_kappa']:
        axes[3].plot(history['val_kappa'], label='Kappa Score', marker='d', color='purple', linewidth=2)
        axes[3].set_title('Validation Kappa Score', fontsize=14, fontweight='bold')
        axes[3].set_xlabel('Epoch', fontsize=12)
        axes[3].set_ylabel('Kappa Score', fontsize=12)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(CONFIG['results_dir'], 'training_metrics.png'), dpi=300, bbox_inches='tight')
    print(f"✓ Saved: {os.path.join(CONFIG['results_dir'], 'training_metrics.png')}")
    plt.close()

def plot_confusion_matrix(y_true, y_pred, classes, title='Confusion Matrix (Spectral Transfer Learning)'):
    """Plot normalized confusion matrix (enhanced style from paper)"""
    cm = confusion_matrix(y_true, y_pred)
    cm_normalized = cm.astype('float') / cm.sum(axis=1)[:, np.newaxis]
    plt.figure(figsize=(12, 10))
    sns.heatmap(cm_normalized, annot=True, fmt='.3f', xticklabels=classes, yticklabels=classes, cmap='Blues',
                cbar_kws={'label': 'Normalized Count'}, annot_kws={"size": 9})
    ...
    print(f"✓ Saved: {os.path.join(CONFIG['results_dir'], 'confusion_matrix.png')}")
    plt.close()
...
def plot_per_class_metrics(y_true, y_pred, classes):
    ...
    ax.set_title('PerClass Performance Metrics (Spectral Transfer Learning)', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ...
    print(f"✓ Saved: {os.path.join(CONFIG['results_dir'], 'per_class_metrics.png')}")
    plt.close()
...
def plot_tsne(model, loader, device, classes, max_samples=1000):
    ...
    with torch.no_grad():
        for i, (images, target) in enumerate(loader):
            if i * loader.batch_size >= max_samples: break
    ...
    for i, cls in enumerate(classes):
        mask = labels == i
        plt.scatter(features_2d[mask, 0], features_2d[mask, 1], label=cls, alpha=0.7, s=50, color=colors[i])
    plt.title('t-SNE Visualization of EfficientNet Features (Spectral Transfer Learning)', fontsize=14, fontweight='bold')
    plt.xlabel('t-SNE Component 1', fontsize=12)
    plt.ylabel('t-SNE Component 2', fontsize=12)
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', fontsize=10)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(CONFIG['results_dir'], 'tsne_visualization.png'), dpi=300, bbox_inches='tight')
    print(f"✓ Saved: {os.path.join(CONFIG['results_dir'], 'tsne_visualization.png')}")
    plt.close()
...
def plot_feature_attention_heatmap(model, loader, device, classes, n_samples=3):
    ...
    plt.suptitle('Feature Attention Maps (Spectral CNN Features)', fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
...
def train_model():
    ...
    try:
        full_dataset = EuroSATSpectralDataset(CONFIG['data_dir'], transform=None)
    except FileNotFoundError as e:
        print(e)
        print("Please ensure EuroSAT RGB data is in the correct directory structure.")
        return
    ...
    model = get_spectral_model(len(full_dataset.classes), in_channels=CONFIG['num_channels'], dropout_rate=CONFIG['dropout_rate'])
    ...
    for epoch in range(CONFIG['epochs']):
        ...
        if val_acc > best_acc:
            ...
            torch.save(model.state_dict(), os.path.join(CONFIG['results_dir'], 'best_spectral_model.pth'))
    ...
    model.load_state_dict(torch.load(os.path.join(CONFIG['results_dir'], 'best_spectral_model.pth')))
